...
//! lives at this crate's root. Everything in this file is deliberately
//! downstream of that boundary: a tap value is always one of the 16 DAC levels
//! or `0.0`, whereas a fade ramp and a filter state are not representable in
//! the recording's palette at all.
//!
...
use crate::HOST_SAMPLE_RATE;

/// The DMG master clock rate the charge exponent below is derived from. Kept
/// as this crate's own constant: the RC network is real-time analog hardware,
/// so its exponent never tracks a machine's live CPU rate (see
/// `charge_per_sample`).
...
pub struct AnalogStage {
    ...
    #[cfg_attr(feature = "serde", serde(skip, default = "dmg_charge_per_sample"))]
    charge: f32,
...
impl AnalogStage {
    ...
    /// of stepping there.
    pub fn fade(&mut self, raw: [f32; 4], dac_on: [bool; 4]) -> [f32; 4] {
    ...
    pub fn high_pass(&mut self, left: f32, right: f32) -> (f32, f32) {
    ...
    /// Test observability only: the per-channel fade levels.
    #[cfg(test)]
    pub(crate) fn fade_state(&self) -> [f32; 4] {
        self.fade
    }
}
...
mod tests {
    ...
    // The DAC's own transfer function is tested at the crate root, which owns
    // it; what remains here is the stage built on top of it. The core keeps
    // the `Hardware`-to-`AnalogModel` coverage test, which needs its enum.

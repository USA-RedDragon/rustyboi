...
column. A "row" is one (ROM, column) case, which is what the 4 counts. The
...
All 4 are floors, not modelling gaps:

- `dma/hdma_timing_fine` (CGB + AGB, 2 rows) — the LCD-on HDMA prefetch term
  is a read-phase correction, not whole-M-cycle elapsed time, and the gambatte
  rows it also serves need the opposite of what this ROM wants. Needs a
  hardware capture of one block's cost read both immediately and a frame later.
- `serial/sc_change_freq_gbc#agb` (1 row) — AGB serial dither; one GBA-SP
  capture cannot separate analog drift from a hidden-bit dependence.
- `timers/tac_set_disabled#agb` (1 row) — the GBA-SP capture self-contradicts:
  TAC bit 3 is a don't-care, so the same write is probed four times and only
  one instance glitches. No deterministic model can match it.

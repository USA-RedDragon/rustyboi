...
def gen_gbmicrotest(roms: Path, out: Path) -> None:
    ...
    write_manifest(
        ...
        [
            ...
            '494/513. 13 display-only ROMs report to VRAM $8000 (or HRAM FF80) not FF82 --',
            'NONE of them call any test_end/test_finish macro (verified against the source),',
            'so they never write an FF82 verdict; they are graded via `mem <addr>=VAL` with a',
            'byte justified from disassembly + real DMG-CPU-08 hardware (NOT from any',
            "emulator's output):",
            ...
            'The 19 remaining fails have NO defensible memory oracle (proven per-test):',
            ' - display-only screen sweeps, NO $8000/FF82 write at all (verdict is only the',
            '   rendered screen, swept by a hand-varied .define): 800/801/802/803-ppu-latch-*,',
            '   oam_sprite_trashing, ppu_scx_vs_bgp, ppu_sprite_testbench, ppu_win_vs_wx,',
            '   ppu_wx_early, toggle_lcdc, wave_write_to_0xC003 (WRAM-only), audio_testbench',
            '   (audio-listen only; $8000 stays power-on, not a hardware assertion),',
            '   temp (a lone `nop` dev stub).',
            ' - $8000 written but not a gradeable verdict: 001-vram_unlocked ($8000 is',
            '   unconditionally $55; the pass signal is only on-screen mid-scanline);',
            '   500-scx-timing / minimal (byte-identical: raw TIMA count of m3 length with',
            '   SCROLL=0, author documented no DMG pass value -- grading it would just trust',
            '   the emulator, i.e. oracle-gaming).',
            ' - 2 DMA-clobber artifacts (400-dma, dma_basic): the ROM copies its DMA routine',
            '   INTO HRAM over FF80-82 (FF80=E0/FF81=46 are the `ldh($46),a` opcode+operand)',
            '   and never writes a verdict; rustyboi == Gambatte.',
            ...
            '001-vram_unlocked: the STAT ISR writes $00 then unconditionally $55 to $8000;',
            'the discriminating result (whether the $00 was visible = VRAM unlocked) exists',
            'only on-screen mid-scanline, never in a stable memory cell -- the final $8000',
            'is $55 regardless of correctness, so there is no gradeable memory verdict.',

...
threshold() {
    case "$1" in
        rustyboi)           echo "53 20" ;;  # first-party RGBDS ROMs (test-roms/); png ROMs settle in <20 frames (mooneye ROMs run to their own done-marker budget, so the SGB ROMs' ~50-frame waits are unaffected)
        acid2)              echo "3 -" ;;

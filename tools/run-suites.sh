...
threshold() {
    case "$1" in
        rustyboi)           echo "50 20" ;;  # first-party RGBDS ROMs (test-roms/); png ROMs settle in <20 frames (mooneye ROMs run to their own done-marker budget, so the SGB ROMs' ~50-frame waits are unaffected)
        acid2)              echo "3 -" ;;
        ...
        docboy_diff_dmg)          echo "518 15" ;;  # 518/531; +6 DMG WX=0 fine-SCX window column advance (window_wx0_scx1..6); 46 docboy-wrong refs excluded
        docboy_diff_cgb)          echo "62 15" ;;   # 62/96; +8 CGB window left-clip chop (window_wx1..6 / wx0_scx0), +8 CGB WX=0 fine-SCX two-column-late takeover (window_wx0_scx1..7 + wx0_scx1_fancy_tile0) + 17 docboy-wrong refs excluded (SameBoy CGB-C+E)

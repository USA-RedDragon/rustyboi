...
threshold() {
    case "$1" in
        rustyboi)           echo "52 20" ;;  # first-party RGBDS ROMs (test-roms/); png ROMs settle in <20 frames (mooneye ROMs run to their own done-marker budget, so the SGB ROMs' ~50-frame waits are unaffected)
        acid2)              echo "3 -" ;;
        ...
        docboy_diff_cgb)          echo "72 15" ;;   # 72/95; -1 denominator: window_wx0_scx1_fancy_tile0 joins the docboy-wrong refs (CGB WX=0 fine-SCX window glitch) -> 18 docboy-wrong refs excluded (SameBoy CGB-C+E)

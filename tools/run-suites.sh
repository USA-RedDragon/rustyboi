...
threshold() {
    case "$1" in
        ...
        gbc_hw_tests)       echo "313 800" ;;  # real-silicon SRAM captures (CGB+DMG+AGB columns); the sram path runs a FLAT budget with no done-marker, so the handful of ROMs that need longer carry a per-test frames= token instead of inflating the whole suite

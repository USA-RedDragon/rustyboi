...
impl Cartridge {
    ...
    pub(super) fn detect_unl_mapper(data: &[u8]) -> UnlMapper {
        ...
        // Super Fighters S: same VF001-Zook challenge-response board as Zook Z,
        // but its boot drives the port through a bespoke copy-and-readback thunk
        // ($14BB) rather than Zook Z's `ld hl,$7081` inline stream, so the arm
        // above cannot see it (and it truthfully declares MBC1, so nothing else
        // routes it either). The observed challenge->response set and the board's
        // synthesised low-memory vectors (see `vf001z_sfs_lowmem`) were recovered
        // from the byte-identical de-protected twin Super Fighters '99, so this is
        // pinned to the one dump they were observed on -- an exact whole-ROM CRC32
        // (behind a cheap 2MB/MBC1-header gate so only that shape pays the checksum).
        if data.len() == 0x20_0000
            && data[CARTRIDGE_TYPE_OFFSET] == 0x01
            && crate::checksum::crc32(data) == super::VF001Z_SFS_CRC32
        {
            return UnlMapper::Vf001Zook(Box::default());
        }
...
/// ever recovered.
const VF001Z_BANK_RESPONSES: [[u8; 4]; 217] = [
...
const VF001Z_CHALLENGE_RESPONSES: &[(u8, &[u8], u8)] = &[
    ...
    // Super Fighters S: the 15 register-0 readbacks its boot verify-loop performs.
    // Each entry builds one of the WRAM far-call `jp` thunks the game runs from;
    // OBSERVED from the de-protected twin Super Fighters '99 -- there the loop is
    // patched to `ret` and the thunk targets are baked into the $CE00-copy source,
    // so each 16-bit response is `baked_jp_target - entry_base` (low byte then high
    // byte across the two consecutive $A080 reads). Not derived; a hardware capture
    // of the real cart's $A080 stream would confirm/replace them.
    (0, &[0x98, 0xA3, 0xA9, 0xCE, 0xCC, 0x40, 0xFF, 0xFF, 0xEF, 0x40, 0xFF, 0xFF, 0xAC, 0x41, 0xFF, 0xFF, 0x8F, 0xA3], 0xCE),
    (0, &[0x98, 0xA3, 0xA9, 0xCE, 0xCC, 0x40, 0xFF, 0xFF, 0xEF, 0x40, 0xFF, 0xFF, 0xAC, 0x41, 0xFF, 0xFF, 0x8F, 0xA3, 0x98], 0xC3),
    (0, &[0x05, 0x1B, 0x0C, 0xC3, 0x7F, 0xE0, 0xE0, 0x2A, 0x5F, 0x2A, 0x9D], 0xE6),
    (0, &[0x05, 0x1B, 0x0C, 0xC3, 0x7F, 0xE0, 0xE0, 0x2A, 0x5F, 0x2A, 0x9D, 0x8D], 0x3B),
    (0, &[0x36, 0x42, 0x3F, 0x12, 0x99, 0xFA, 0x0A, 0x0A, 0x06, 0x06, 0xE6, 0x67, 0x7C, 0x62, 0xB5], 0x46),
    (0, &[0x36, 0x42, 0x3F, 0x12, 0x99, 0xFA, 0x0A, 0x0A, 0x06, 0x06, 0xE6, 0x67, 0x7C, 0x62, 0xB5, 0x8D], 0x9B),
    (0, &[0x67, 0x79, 0x8C, 0x75, 0x9E, 0x73, 0x0B, 0x78, 0x52, 0xDF, 0xFF, 0x99, 0xBF], 0xEE),
    (0, &[0x67, 0x79, 0x8C, 0x75, 0x9E, 0x73, 0x0B, 0x78, 0x52, 0xDF, 0xFF, 0x99, 0xBF, 0x88], 0xEE),
    (0, &[0x98, 0xA3, 0xA9, 0x4C, 0xE8, 0xCF, 0x11, 0x81, 0x70, 0xCD, 0xBB, 0x14, 0xFA, 0xFF, 0x7F, 0x88], 0x4C),
    (0, &[0x98, 0xA3, 0xA9, 0x4C, 0xE8, 0xCF, 0x11, 0x81, 0x70, 0xCD, 0xBB, 0x14, 0xFA, 0xFF, 0x7F, 0x88, 0x99], 0xA8),
    (0, &[0xC9, 0xD7, 0xCC, 0x38, 0x02, 0x30, 0xF0, 0x04, 0x30, 0xF8, 0x06, 0x30, 0xBE], 0x94),
    (0, &[0xC9, 0xD7, 0xCC, 0x38, 0x02, 0x30, 0xF0, 0x04, 0x30, 0xF8, 0x06, 0x30, 0xBE, 0x8B], 0xEE),
    (0, &[0xFA, 0x01, 0x05, 0x2D, 0xE5, 0xD5, 0xC5, 0xF0, 0xE0, 0xF5, 0x21, 0x9E], 0x05),
    (0, &[0xFA, 0x01, 0x05, 0x2D, 0xE5, 0xD5, 0xC5, 0xF0, 0xE0, 0xF5, 0x21, 0x9E, 0x8F], 0x7F),
    (0, &[0x2B, 0x35, 0x4C, 0xD5, 0x30, 0xE8, 0x08, 0x30, 0xF0, 0x0A, 0x30, 0xF8, 0x0C, 0x95], 0x70),
    (0, &[0x2B, 0x35, 0x4C, 0xD5, 0x30, 0xE8, 0x08, 0x30, 0xF0, 0x0A, 0x30, 0xF8, 0x0C, 0x95, 0x9C], 0xEE),
    (0, &[0x5C, 0x61, 0x6F, 0xEE, 0x81, 0x68, 0x40, 0xDC, 0xFE, 0xDE, 0x01, 0x7E, 0xF4, 0x9C, 0x83, 0x9D, 0xD8, 0x7B, 0xF1, 0x89], 0x6C),
    (0, &[0x5C, 0x61, 0x6F, 0xEE, 0x81, 0x68, 0x40, 0xDC, 0xFE, 0xDE, 0x01, 0x7E, 0xF4, 0x9C, 0x83, 0x9D, 0xD8, 0x7B, 0xF1, 0x89, 0xB8], 0x2F),
    (0, &[0x8D, 0x93, 0x95, 0x8D, 0x33, 0x42, 0x00, 0xF0, 0x07, 0x20, 0xF0, 0x00, 0x20, 0xF8, 0x02, 0x20, 0x00, 0x8B], 0x64),
    (0, &[0x8D, 0x93, 0x95, 0x8D, 0x33, 0x42, 0x00, 0xF0, 0x07, 0x20, 0xF0, 0x00, 0x20, 0xF8, 0x02, 0x20, 0x00, 0x8B, 0xAB], 0xFA),
    (0, &[0xBE, 0xC0, 0xC0, 0xFA, 0x09, 0x09, 0x09, 0x01, 0xF0, 0xCE, 0x2A, 0x02, 0x03, 0x2A, 0x02, 0x03, 0x83], 0x7A),
    (0, &[0xBE, 0xC0, 0xC0, 0xFA, 0x09, 0x09, 0x09, 0x01, 0xF0, 0xCE, 0x2A, 0x02, 0x03, 0x2A, 0x02, 0x03, 0x83, 0x9E], 0xA7),
    (0, &[0xEF, 0xF1, 0x0C, 0xB8, 0xFF, 0x1F, 0xDE, 0x0F, 0xB6], 0x64),
    (0, &[0xEF, 0xF1, 0x0C, 0xB8, 0xFF, 0x1F, 0xDE, 0x0F, 0xB6, 0x8C], 0x5D),
    (0, &[0x20, 0x3E, 0x20, 0x36, 0x3C, 0x9E, 0xFC, 0x7A, 0xE1, 0x97, 0x9B, 0xF7, 0xFE, 0x9E], 0xAD),
    (0, &[0x20, 0x3E, 0x20, 0x36, 0x3C, 0x9E, 0xFC, 0x7A, 0xE1, 0x97, 0x9B, 0xF7, 0xFE, 0x9E, 0x8A], 0xD7),
    (0, &[0x51, 0x65, 0x66, 0xA5, 0xFF, 0x9D, 0x0B, 0x05, 0xFB, 0x67, 0x8F], 0x75),
    (0, &[0x51, 0x65, 0x66, 0xA5, 0xFF, 0x9D, 0x0B, 0x05, 0xFB, 0x67, 0x8F, 0xAD], 0x0F),
    (0, &[0xE0, 0xFE, 0xE0, 0x2F, 0xF8, 0x04, 0x30, 0x00, 0x06, 0x40, 0xE8, 0x08, 0x40, 0xF0, 0x0A, 0x40, 0xF8, 0x0C, 0x40, 0x00, 0xA4], 0x14),
    (0, &[0xE0, 0xFE, 0xE0, 0x2F, 0xF8, 0x04, 0x30, 0x00, 0x06, 0x40, 0xE8, 0x08, 0x40, 0xF0, 0x0A, 0x40, 0xF8, 0x0C, 0x40, 0x00, 0xA4, 0x9B], 0x20),
];

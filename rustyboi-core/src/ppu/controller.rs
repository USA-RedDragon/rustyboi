...
mod tests {
    ...
    // --- Window vs SCX fine-scroll regression (full-width WX==7 HUD bar shifting
    // with SCX) --------------------------------------------------------------
    //
    // The window is locked to screen coordinates and must ignore SCX. A WX==7
    // full-width window (the HUD bars in Pokémon Crystal / Bomberman) triggers at
    // LX==0, after the SCX&7 fine-scroll discard has consumed the leading
    // BACKGROUND pixels — so it draws unshifted. The bug had the discard pop the
    // window's own pixels, shifting the bar left by SCX&7 (moving it with the
    // camera one frame per horizontal scroll). This drives a synthetic ROM whose
    // tile marks the first column of every 8-px tile and asserts the window marks
    // stay tile aligned under SCX, while a plain background with the same tile
    // DOES shift (proving the discard still works and the fix is not over-broad).

    /// Assemble a tiny DMG ROM: fill tile #2 with a dark marker in column 0 of
    /// each 8-px tile, tile the whole screen with it, set `SCX`, and either enable
    /// the window at `WX==7` (`window`) or leave it off (plain BG). Run a few
    /// frames and return the x positions of the dark (color-3) pixels on a
    /// mid-screen row.
    fn window_scx_dark_columns(window: bool, scx: u8) -> Vec<usize> {
        use crate::cartridge::Cartridge;
        use crate::gb::{Frame, Hardware, GB};
        // Two-pass mini-assembler over a fixed layout so the loop jumps resolve.
        let base: u16 = 0x150;
        let lcdc: u8 = if window { 0xB1 } else { 0x91 }; // +window vs BG only
        // (label, code) pairs; `J(op, target)` is a 3-byte absolute jump.
        enum I {
            R(Vec<u8>),
            L(&'static str),
            J(u8, &'static str),
        ...
        let prog = vec![
            R(vec![0xF3]), // DI
            L("w"),
            R(vec![0xF0, 0x44, 0xFE, 0x90]), // LDH A,(LY); CP 144
            J(0xC2, "w"),                    // JP NZ w  (wait for VBlank)
            R(vec![0xAF, 0xE0, 0x40]),       // XOR A; LDH (LCDC),A  (LCD off)
            // tile #2 @ 0x8020: 8 rows of low=0x80, high=0x80 -> col0 = color 3
            R(vec![0x21, 0x20, 0x80, 0x0E, 0x08, 0x3E, 0x80]), // LD HL,8020; LD C,8; LD A,80
            L("t"),
            R(vec![0x22, 0x22, 0x0D]), // LD (HL+),A x2; DEC C
            J(0xC2, "t"),
            // tilemap 0x9800..0x9C00 = tile 2
            R(vec![0x21, 0x00, 0x98, 0x01, 0x00, 0x04, 0x16, 0x02]), // LD HL,9800; LD BC,0400; LD D,2
            L("m"),
            R(vec![0x7A, 0x22, 0x0B, 0x78, 0xB1]), // LD A,D; LD (HL+),A; DEC BC; LD A,B; OR C
            J(0xC2, "m"),
            // registers: WY=0, SCX=scx, WX=7, BGP=0xE4, LCDC
            R(vec![0xAF, 0xE0, 0x4A]),        // XOR A; LDH (WY),A
            R(vec![0x3E, scx, 0xE0, 0x43]),   // LD A,scx; LDH (SCX),A
            R(vec![0x3E, 0x07, 0xE0, 0x4B]),  // LD A,7; LDH (WX),A
            R(vec![0x3E, 0xE4, 0xE0, 0x47]),  // LD A,E4; LDH (BGP),A
            R(vec![0x3E, lcdc, 0xE0, 0x40]),  // LD A,lcdc; LDH (LCDC),A
            L("s"),
            J(0xC3, "s"), // spin
        ];
        // Pass 1: addresses.
        ...
        for ins in &prog {
            match ins {
                R(b) => addr += b.len() as u16,
                L(l) => {
                    labels.insert(*l, addr);
                }
                J(..) => addr += 3,
            }
        }
        ...
        for ins in &prog {
            match ins {
                R(b) => code.extend_from_slice(b),
                L(_) => {}
                J(op, l) => {
                    let t = labels[l];
                    code.push(*op);
                    code.push((t & 0xFF) as u8);
                    code.push((t >> 8) as u8);
                }
            }
        }
        ...
        let cart = Cartridge::from_bytes(&rom).expect("load synthetic ROM");
        let mut gb = GB::new(Hardware::DMG);
        gb.insert(cart);
        gb.skip_bios();
        // The first frames after the ROM's LCD-off setup are warmup-blank; the
        // window renders stably from ~frame 4, so settle a few extra.
        let mut frame = None;
        for _ in 0..8 {
            frame = Some(gb.run_until_frame(false).0);
        }
        let buf = match frame.unwrap() {
            Frame::Monochrome(b) => b,
            Frame::Color(_) => panic!("DMG frame expected"),
        };
        let row = 40; // any window/BG row
        (0..160).filter(|&x| buf[row * 160 + x] == 3).collect()
    }

    #[test]
    fn window_ignores_scx_fine_scroll() {
        // Baseline: with SCX=0 the dark marker sits at the left of every 8-px tile
        // for both the window and the plain background.
        assert_eq!(window_scx_dark_columns(true, 0), (0..160).step_by(8).collect::<Vec<_>>());
        assert_eq!(window_scx_dark_columns(false, 0), (0..160).step_by(8).collect::<Vec<_>>());

        // The window is immune to SCX: at SCX=3 its marks stay tile aligned
        // (x % 8 == 0). Pre-fix they shifted to x % 8 == 5.
        let win = window_scx_dark_columns(true, 3);
        assert!(!win.is_empty());
        assert!(win.iter().all(|&x| x % 8 == 0), "window shifted by SCX: {win:?}");

        // The background is NOT immune — SCX=3 still discards its leading pixels,
        // shifting its marks (x % 8 == 5). This proves the fix is scoped to the
        // window and did not disable the background fine-scroll discard.
        let bg = window_scx_dark_columns(false, 3);
        assert!(!bg.is_empty());
        assert!(bg.iter().all(|&x| x % 8 == 5), "background discard broke: {bg:?}");
    }
}

...
| The CGB boot ROM's DMG-compat path runs the title-hash palette lookup only for a Nintendo licensee, which lengthens boot and moves post-boot DIV | mooneye (cgb0/cgb/cgbe/agb, DMG-compat cart) | **ROM** | `boot/licensee_boot_div` — 16 ROMs, 4 revisions x 4 header licensee configs (`_l00`, `_l33`, `_l01`, `_l33n01`), assembled from one source and differing only in the bytes `rgbfix -l`/`-k` write. Pan Docs "CGB Registers"/compatibility palettes; the branch is the literal `21 4B 01 / 7E / FE 33 / 20 0B` at image offset 0x0579 (0x0573 on CGB0) in all four images. Expected DIV derived by executing each genuine image against each built cart — all 16 pass under `--real-bios`; the 8 Nintendo-arm rows fail on the current HLE, which flattens the lookup to a constant. Lookup cost is TITLE-dependent (+808 YOSHI .. +5476 METROID2 T cycles), so the title `RBLICENSEE` (a deliberate table miss, +4360) is pinned in the Makefile and is part of the fixture. The 4 T `$014B != $33` sub-arm is deliberately ungraded: 4 T is 1/64 of a DIV tick. Blind spot closed: no public suite carries a Nintendo licensee code, so the whole ~6,800-case battery is structurally unable to reach this branch. |

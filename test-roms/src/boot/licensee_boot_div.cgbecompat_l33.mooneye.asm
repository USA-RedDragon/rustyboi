; licensee_boot_div.cgbecompat_l33.mooneye — the CGB boot ROM's licensee
; branch, observed as post-boot DIV on CPU CGB-D/E silicon.
;
; A DMG-compatibility cartridge (non-Nintendo header) booted on CPU CGB-D/E. The
...
;
; Expected DIV $26, derived by executing cgbE_boot.bin against this exact
...
; `rgbfix` from the `cgbecompat_l33` model token in the Makefile, so the

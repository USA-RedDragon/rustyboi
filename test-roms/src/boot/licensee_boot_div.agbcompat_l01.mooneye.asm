; licensee_boot_div.agbcompat_l01.mooneye — the CGB boot ROM's licensee
; branch, observed as post-boot DIV on AGB silicon.
;
; A DMG-compatibility cartridge (Nintendo header) booted on AGB. The
...
;
; This variant: old licensee $01 IS the Nintendo marker, so the boot ROM hashes
; the title and walks its checksum table.
;
...
;
; Expected DIV $37, derived by executing agb_boot.bin against this exact
...
; `rgbfix` from the `agbcompat_l01` model token in the Makefile, so the
...
Start:
    ...
    licensee_boot_div_test $37, $01, $00, $00
